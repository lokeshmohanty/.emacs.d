...
The configuration files are ~early-init.el~ and ~init.el~. ~README.org~ is the literate config
from which the two configuration files are generated using =org-babel-tangle=.

To use this config, clone this repository in your home directory as ~.emacs.d~.

#+begin_src shell
  cd && git clone https://github.com/lokeshmohanty/.emacs.d.git
#+end_src

or in your ~XDG_CONFIG_HOME~ as ~emacs~

#+begin_src shell
  cd ~/.config && git clone https://github.com/lokeshmohanty/.emacs.d.git emacs
#+end_src

The literate documentation of the code starts from the [[#early-init][Early Init]] section.
I use a custom build of emacs. The steps to get the same build are documented in the following section.
...
*Note:* I am currently using =NixOS= and using the ~emacs-pgtk~ package. Hence the above section
might not be upto date. It will be removed once my emacs setup on =NixOS= is complete.
...
*** Pdf in new frame (should already exist)

References: [[https://emacs.stackexchange.com/a/55405][stack-overflow]]
...
*** LSP
*References*: [[https://emacs-lsp.github.io/lsp-mode/][lsp]], [[https://emacs-lsp.github.io/lsp-mode/page/lsp-cmake/][cmake]], [[https://emacs-lsp.github.io/lsp-mode/tutorials/how-to-turn-off/][lsp features]]
...
** REST Client (verb, restclient)
*References*: [[https://github.com/federicotdn/verb][verb]]
